...
for ($i = 0; $i < count($fc_detail_data) - 1; $i ++) {
    if (trim($fc_detail_data[$i][6]) != '' && trim($fc_detail_data[$i][7]) != '' && trim($fc_detail_data[$i][9]) != '') {
        if ($fc_detail_data[$i][8] == '') {
            ...
            $arrayFcDetail['FC'][$i]['PROJECT_NAME'] = $fc_detail_data[$i][3];
            $arrayFcDetail['FC'][$i]['STATUS'] = trim($fc_detail_data[$i][9]);
            $arrayFcDetail['FC'][$i]['STANDARD_DURATION'] = '';
            $arrayFcDetail['FC'][$i]['DELIVERY_DATE'] = $fc_detail_data[$i][7];;
    ...
    if (trim($fc_data[$i][10]) != '' && trim($fc_data[$i][11]) != '' && trim($fc_data[$i][0]) != '') {
        if ($fc_data[$i][8] == '') {
            ...
            foreach($arrayFcDetail['FC'] as $a => $b) {
                ...
                if($arrayFcDetail['FC'][$a]['PROJECT_NO'] == $fc_data[$i][1] && $arrayFcDetail['FC'][$a]['PROJECT_NAME'] == $fc_data[$i][2]) {
...
for ($i = 0; $i < count($newcoding_detail_data) - 1; $i ++) {
    if (trim($newcoding_detail_data[$i][7]) != '' && trim($newcoding_detail_data[$i][7]) != '-' && trim($newcoding_detail_data[$i][9]) != '') {
...
for ($i = 0; $i < count($newcoding_data) - 1; $i ++) {
    if (trim($newcoding_data[$i][9]) != '' && trim($newcoding_data[$i][11]) != '' && trim($newcoding_data[$i][12]) != '') {
            ...
            if($newcoding_data[$i][3] == 'Option') {
                $newcoding_data[$i][3] = 'New';
            }
            foreach($arrayNewCodingDetail['NewCoding'] as $a => $b) {
                if(trim($arrayNewCodingDetail['NewCoding'][$a]['PROJECT_NO']) == trim($newcoding_data[$i][1]) && trim($arrayNewCodingDetail['NewCoding'][$a]['PROJECT_NAME']) == trim($newcoding_data[$i][2]) && trim($arrayNewCodingDetail['NewCoding'][$a]['TYPE']) == trim($newcoding_data[$i][3])) {
